...
class HookInstaller:
    ...
    def _print_post_install_instructions(self, api_key: str) -> None:
        ...
        print(f"{Colors.CYAN}{'=' * 60}{Colors.NC}")
        print()
        print(f"{Colors.YELLOW}1. API Key — add this to your MCP server env in ~/.claude.json:{Colors.NC}")
        print(f'   "env": {{')
        print(f'     "MCP_API_KEY": "{api_key}",')
        print(f'     "MCP_HTTP_ENABLED": "true",')
        print(f'     "MCP_HTTP_PORT": "8000"')
        print(f'   }}')
        print()
        print(f"{Colors.YELLOW}2. HTTP + stdio dual-server — update your MCP command in ~/.claude.json:{Colors.NC}")
        print("   The hooks use the HTTP API (port 8000) for writes, but the default")
        print("   MCP config only starts the stdio server. Use a bash wrapper:")
        print()
        print('   "memory": {')
        print('     "command": "bash",')
        print('     "args": [')
